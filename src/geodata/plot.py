...
def time_series(
    ...
    **kwargs,
    ...
    if agg_time_method == "mean":
        ...
        raise NotImplementedError(f"agg_time_method {agg_time_method} is not supported.")

    if not agg_slice and lat_slice is None and lon_slice is None:
        if agg_slice:
            raise RuntimeError("agg_slice cannot be set to True without lat_slice or lon_slice.")
