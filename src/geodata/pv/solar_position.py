...
def SolarPosition(ds):
    ...
    lmst = (6.697375 + (ds["time.hour"] + ds["time.minute"] / 60.0) + 0.0657098242 * n) * 15.0 + ds[
        "lon"
    ...
    alt = np.arcsin(
        (np.sin(lat) * np.sin(dec) + np.cos(lat) * np.cos(dec) * np.cos(h)).clip(min=-1.0, max=1.0)
    ).rename(
        "altitude"
    )  # pylint: disable=no-member

    az = np.arccos(
        ((np.sin(dec) * np.cos(lat) - np.cos(dec) * np.sin(lat) * np.cos(h)) / np.cos(alt)).clip(
            min=-1.0, max=1.0
        )
    )  # pylint: disable=no-member
    ...
    if "influx_toa" in ds:
        ...
        # [3]
        atmospheric_insolation = (1366.1 * (1 + 0.033 * np.cos(g)) * np.sin(alt)).rename(
            "atmospheric insolation"
        )

    solar_position = xr.Dataset({da.name: da for da in [alt, az, atmospheric_insolation]})
